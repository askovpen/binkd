 ...
 */
/*
 * $Id: compress.c,v 2.4 2004/01/08 12:46:01 val Exp $
 *
 * $Log: compress.c,v $
  ...
  switch (type) {
    ...
    case 2: {
      *data = calloc(1, sizeof(bz_stream));
      if (*data == NULL) {
        Log (LL_ERR, "compress_init: not enough memory (%lu needed)", sizeof(bz_stream));
        return BZ_MEM_ERROR;
      }
    ...
    case 1: {
      *data = calloc(1, sizeof(z_stream));
      if (*data == NULL) {
        Log (LL_ERR, "compress_init: not enough memory (%lu needed)", sizeof(z_stream));
        return Z_MEM_ERROR;
      }
...
#endif
    default:
      Log (LL_ERR, "Unknown compression method: %d; data lost", type);
  }
  return -1;
}
...
void compress_deinit(int type, void *data)
{
  switch (type) {
    ...
    case 2: {
      ...
      if (rc < 0) Log (LL_ERR, "BZ2_bzCompressEnd error: %d", rc);
      break;
    }
#endif
#ifdef WITH_ZLIB
    case 1: {
      ...
      if (rc < 0) Log (LL_ERR, "deflateEnd error: %d", rc);
      break;
    }
#endif
    default:
      Log (LL_ERR, "Unknown compression method: %d", type);
  }
  free(data);
  return;
}
void compress_abort(int type, void *data)
{
  ...
  if (data) { 
    Log (LL_MINOR, "Purge compress buffers");
    do {
      i=sizeof(buf);
      j=0;
...
int decompress_init(int type, void **data)
{
  switch (type) {
    ...
    case 2: {
      *data = calloc(1, sizeof(bz_stream));
      if (*data == NULL) {
        Log (LL_ERR, "decompress_init: not enough memory (%lu needed)", sizeof(bz_stream));
        return BZ_MEM_ERROR;
      }
    ...
    case 1: {
      *data = calloc(1, sizeof(z_stream));
      if (*data == NULL) {
        Log (LL_ERR, "decompress_init: not enough memory (%lu needed)", sizeof(z_stream));
        return Z_MEM_ERROR;
      }
...
int decompress_abort(int type, void *data) {
  ...
  if (data) {
    Log (LL_MINOR, "Purge decompress buffers");
    do {
      i=sizeof(buf);
      j=0;
