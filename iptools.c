...
/*
 * $Id: iptools.c,v 2.19 2012/01/22 00:19:47 green Exp $
 *
 * $Log: iptools.c,v $
...
void setsockopts (SOCKET s)
{
  ...
  if (ioctl (s, FIONBIO, (char *) &arg, sizeof arg) < 0)
    Log (LL_CRIT, "ioctl (FIONBIO): %s", TCPERR ());
  ...
  if (ioctlsocket (s, FIONBIO, &arg) < 0)
    if (!binkd_exit)
      Log (LL_CRIT, "ioctlsocket (FIONBIO): %s", TCPERR ());
#endif
#endif

#if defined(UNIX) || defined(EMX) || defined(AMIGA)
  if (fcntl (s, F_SETFL, O_NONBLOCK) == -1)
    Log (LL_CRIT, "fcntl: %s", strerror (errno));
#endif
}
...
char * find_port (char *s)
{
  ...
  if (ps == NULL)
    Log (LL_CRIT, "%s: incorrect port (getaddrinfo: %s)", s, gai_strerror(aiErr));
...
int sockaddr_cmp_addr(const struct sockaddr *a, const struct sockaddr *b)
{
  ...
  {
    Log (LL_ERR, "Unsupported address family: %d", a->sa_family);
    return -1;
  }
}
