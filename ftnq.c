...
/*
 * $Id: ftnq.c,v 2.39 2011/05/06 15:44:04 gul Exp $
 *
 * $Log: ftnq.c,v $
...
/* `FOO:/bar means FOO:\..\bar on Amiga */
	...
	strcat (outb_path, PATH_SEPARATOR);
      ...
      {
	Log (LL_CRIT, "cannot opendir %s: %s", outb_path, strerror (errno));
	continue;
      }
...
static FTNQ *q_scan_box (FTNQ *q, FTN_ADDR *fa, char *boxpath, char flvr, int deleteempty, BINKD_CONFIG *config)
{
  ...
  if (access(boxpath, R_OK | W_OK) != 0) {
    if (access(boxpath, F_OK) == 0)
      Log (LL_CRIT, "No access to filebox `%s'", boxpath);
    return q;
  }
#endif
  s = buf + strlen (buf);
  if ((dp = opendir (boxpath)) != NULL)
  {
    ...
    if (n_files == 0 && deleteempty) {
      if (rmdir (boxpath) == 0)
        Log (LL_WARN, "Empty filebox %s deleted", boxpath);
      else
        Log (LL_CRIT, "Cannot delete empty filebox %s: %s", boxpath, strerror (errno));
    }
  }
  return q;
}
...
static void process_bsy (FTN_ADDR *fa, char *path, BINKD_CONFIG *config)
{
      ...
      && time (0) - sb.st_mtime > config->kill_old_bsy)
  {
    ...
    Log (LL_ERR, "found old %s file for %s", s, buf);
    delete (path);
  }
  else
  {
...
static FTNQ *q_add_dir (FTNQ *q, char *dir, FTN_ADDR *fa1, BINKD_CONFIG *config)
{
  ...
  if ((dp = opendir (dir)) != 0)
  {
    ...
    Log (LL_CRIT, "cannot opendir %s: %s", dir, strerror (errno));
  return q;
}
...
FTNQ *q_add_file (FTNQ *q, char *filename, FTN_ADDR *fa1, char flvr, char action, char type, BINKD_CONFIG *config)
{
  ...
  if (q != SCAN_LISTED)
  {
    ...
    { char *p;
      ...
      f = fopen(filename, "r");
      if (f == NULL)
      { Log (LL_CRIT, "Can't open %s: %s", filename, strerror(errno));
        return q;
      }
      if (!fgets(str, sizeof(str), f))
      { Log (LL_CRIT, "Incorrect status (can't fgets), ignored");
        fclose(f);
        return q;
      }
      fclose(f);
      if (*str && isspace(*str))
      { Log (LL_CRIT, "Incorrect status (space first), ignored");
        return q;
      }
      for (p=str+strlen(str)-1; isspace(*p); *p--='\0');
      Log (LL_ERR, "Status is '%s'", str);
      if (!parse_args (argc, argv, str, "Status"))
      { Log (LL_CRIT, "Incorrect status, ignored");
        return q;
      }
    }
...
int create_poll (FTN_ADDR *fa, int flvr, BINKD_CONFIG *config)
{
  ...
  if (*buf)
  {
    ...
    if ((rc = create_empty_sem_file (buf)) == 0)
      if (errno != EEXIST)
        Log (LL_CRIT, "cannot create %s: %s", buf, strerror (errno));
  }
  else
    Log (LL_CRIT, "%s: unknown domain", fa->domain);
  return rc;
}
...
void hold_node (FTN_ADDR *fa, time_t hold_until, BINKD_CONFIG *config)
{
  ...
  ftnaddress_to_filename (buf, fa, config);
  Log (LL_ERR, "holding %s (%s)", addr, time);
  if (*buf)
  {
    FILE *f;
    FTN_NODE *fn;
