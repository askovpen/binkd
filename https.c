...
int h_connect(int so, char *host, BINKD_CONFIG *config, char *proxy, char *socks)
{
	...
	if (proxy[0])
	{
		...
		if ((sp=strchr(buf, '/')) != NULL)
			*sp++ = '\0';
		Log (LL_NOTICE, "connected to proxy %s", buf);
		if(sp) 
		{
			char *sp1;
#ifdef NTLM
		...
		{
			Log (LL_NOTICE, "Send to proxy error: %s", TCPERR());
			SetTCPError(PR_ERROR);
			return 1;
		}
		Log (LL_DBG5, "sent proxy sockfd %d request: %s", so, buf);
		for(i=0; i<sizeof(buf)-1; i++)
		{
			...
			{
				if (n<0)
					Log (LL_NOTICE, "proxy error: %s", TCPERR());
				else
					Log (LL_NOTICE, "proxy timeout...");
				SetTCPError(PR_ERROR);
				return 1;
			}
			...
			{
				if (n<0)
					Log (LL_ERR, "Proxy error: %s", TCPERR());
				else
					Log (LL_ERR, "Connection closed by proxy...");
				SetTCPError(PR_ERROR);
				return 1;
			}
			buf[i+1]=0;
			if((i+2>=sizeof(buf))||((sp=strstr(buf, "\r\n\r\n"))!=NULL)||((sp=strstr(buf, "\n\n"))!=NULL))
			{
				...
				if ((ntlm) && ((sp=strstr(buf, "uthenticate: NTLM "))!=NULL))
				{
					...
					free(sp);
					if (i) Log (LL_ERR, "Invalid username/password/host/domain string (%s) %d", ntlmsp, i);
					free(ntlmsp);
				...
				Log (LL_ERR, "Connection rejected by proxy (%s)", buf);
				SetTCPError(PR_ERROR);
				return 1;
			}
		}
	...
	if (socks[0])
	{
		...
		if ((sauth=strchr(buf, '/')) != NULL)
			*sauth++ = '\0';
		Log (LL_NOTICE, "connected to socks%c %s", sauth ? '5' : '4', buf);
		...
		if (!sauth) /* SOCKS4 */
		{
			...
			{
				Log (LL_ERR, "getaddrinfo failed: %s (%d)", gai_strerror(aiErr), aiErr);
				SetTCPError(PR_ERROR);
				return 1;
			}
		}
		else	    /* SOCKS5 */
		{
			...
			{
				Log (LL_CRIT, "Auth. method not supported by socks5 server");
				free(sauth);
				freeaddrinfo(aiHead);
				SetTCPError(PR_ERROR);
				return 1;
			...
			if (buf[1]==2) /* username/password method */
			{
				...
				{
					Log (LL_CRIT, "Authentication failed (socks5 returns %02X%02X)", (unsigned char)buf[0], (unsigned char)buf[1]);
					free(sauth);
					freeaddrinfo(aiHead);
					SetTCPError(PR_ERROR);
					return 1;
		...
		for (ai = aiHead; ai != NULL; ai = ai->ai_next)
		{
			...
			if (!sauth) /* SOCKS4 */
			{
				...
				buf[1]=1;
				lockhostsem();
				Log (LL_NOTICE, strcmp(port, config->oport) == 0 ? "trying %s..." : "trying %s:%u...",
			...
			for (i=0; i<sizeof(buf); i++)
			{
				...
				{
					if (n<0)
						Log (LL_NOTICE, "socks error: %s", TCPERR());
					else
						Log (LL_NOTICE, "socks timeout...");
					if (sauth) free(sauth);
					freeaddrinfo(aiHead);
					SetTCPError(PR_ERROR);
					return 1;
				...
				if ((n=recv(so, buf+i, 1, 0))<1) {
					if (n<0)
						Log (LL_ERR, "socks error: %s", TCPERR());
						Log (LL_ERR, "connection closed by socks server...");
					if (sauth) free(sauth);
					freeaddrinfo(aiHead);
					SetTCPError(PR_ERROR);
					return 1;
				...
				{
					if (buf[0]!=0) {
						Log (LL_ERR, "Bad reply from socks server");
						freeaddrinfo(aiHead);
						SetTCPError(PR_ERROR);
						return 1;
					}
					if (buf[1]!=90) {
						Log (LL_ERR, "connection rejected by socks4 server (%d)", (unsigned char)buf[1]);
						SetTCPError(PR_ERROR);
						break; /* try next IP */
					}
					else {
				...
				else if (sauth && i>5)
				{
					...
					switch (buf[1])
					{
						case 1: Log (LL_ERR, "general SOCKS5 server failure"); break;
						case 2: Log (LL_ERR, "connection not allowed by ruleset (socks5)"); break;
						case 3: Log (LL_ERR, "Network unreachable (socks5)"); break;
						case 4: Log (LL_ERR, "Host unreachable (socks5)"); break;
						case 5: Log (LL_ERR, "Connection refused (socks5)"); break;
						case 6: Log (LL_ERR, "TTL expired (socks5)"); break;
						case 7: Log (LL_ERR, "Command not supported by socks5"); break;
						case 8: Log (LL_ERR, "Address type not supported"); break;
						default: Log (LL_ERR, "Unknown reply (0x%02X) from socks5 server", (unsigned char)buf[1]);
					}
					SetTCPError(PR_ERROR);
					return 1;
				}
