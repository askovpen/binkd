...
/*
 * $Id: readcfg.c,v 2.104 2012/06/17 00:21:40 green Exp $
 *
 * $Log: readcfg.c,v $
...
void unlock_config_structure(BINKD_CONFIG *c, int on_exit)
{
  ...
  if (usage == 0)
  {
    ...
    if (c != &work_config && !binkd_exit)
      Log(LL_MINOR, "previous config is no longer in use, unloading");
...
#define MAX_CONFIGERROR_PARAMS 6
  ...
  Log(LL_ERR, format, data[0], data[1], data[2], data[3], data[4], data[5]);
  va_end(args);
  return 0;
}
...
static void add_to_config_list(const char *path, FILE *f)
{
  ...
  {
    Log(LL_WARN, "Cannot get modification time for %s: %s", path, strerror(errno));
    new_entry.mtime = 0;
  }
...
BINKD_CONFIG *readcfg (char *path)
{
  ...
  if (!new_config)
  {
    ...
    if (current_config)
    {
      Log(LL_ERR, "error in configuration, using old config");
...
int checkcfg(void)
{
  ...
  Log(LL_WARN, "Reloading configuration...");
  ...
  if (new_config && new_config->perl_script[0])
  {
    if (!perl_init(new_config->perl_script, new_config))
      ...
      {
        Log (LL_ERR, "error parsing Perl script %s, using old config", new_config->perl_script);
...
static int read_aka_list (KEYWORD *key, int wordcount, char **words)
{
  ...
  for (i = 0; i < wordcount; i++)
  {
    ...
    {
      Log (LL_WARN, "Duplicate address %s in config ignored", words[i]);
      continue;
    }
...
static int read_skip (KEYWORD *key, int wordcount, char **words)
{
  ...
  if ((destr = key->option1) != 0)
    Log(LL_ERR, "%s: line %d: warning: option skipmask is obsolete, use skip instead", current_path, current_line);
  for (i = 0; i < wordcount; i++)
  {
    char *w = words[i];
