...
/*
 * $Id: inbound.c,v 2.45 2009/06/12 17:42:56 gul Exp $
 *
 * $Log: inbound.c,v $
...
static int creat_tmp_name (char *s, TFILE *file, FTN_ADDR *from, char *inbound)
{
  ...
  while (1)
  {
    ...
    if (create_empty_sem_file (s))
    {
      if ((f = fopen (s, "w")) == 0)
      {
        Log (LL_CRIT, "%s: %s", s, strerror (errno));
        delete (s);
        return 0;
      }
...
static int to_be_deleted (char *tmp_name, char *netname, off_t filesize, BINKD_CONFIG *config)
{
  ...
  {
    Log (LL_NOTICE, "found old .dt/.hr files for %s", netname);
    return 1;
  }
...
static int find_tmp_name (char *s, TFILE *file, STATE *state, BINKD_CONFIG *config)
{
  ...
  {
    Log (LL_CRIT, "cannot opendir %s: %s", inbound, strerror (errno));
    return 0;
  }
  ...
  while ((de = readdir (dp)) != 0)
  {
    ...
    if ((f = fopen (s, "r")) == NULL)
    {
      Log (LL_CRIT, "find_tmp_name: %s: %s", de->d_name, strerror (errno));
    }
    ...
    {  /* This .hr is empty, now checks to old */
      ...
      {
        Log (LL_INFO, "old empty partial file %s is removed", de->d_name);
        remove_hr (s);
      }
    }
    else
    {
      ...
      if (!w[3])
      {
        if (to_be_deleted (s, "unknown", (off_t)-1, config))
        {
          Log (LL_INFO, "old partial file %s with garbage is removed", de->d_name);
          remove_hr (s);
        }
      }
      else
      {
        ...
        if (file == NULL)
        {
          ...
          {
            Log (LL_INFO, "partial file %s removed", w[0]);
            remove_hr (s);
          }
        }
        else if (i >= 0 && !strcmp (w[0], file->netname))
        {
          ...
          { /* non-destructive skip file from busy aka */
            if (i >= state->nfa)
            {
              Log (LL_ERR, "Skip partial file %s: aka %s busy", w[0], w[3]);
              for (i = 0; i < 4; ++i)
                xfree (w[i]);
              return 0;
            }
          ...
          {
            Log (LL_INFO, "dup partial file %s removed", w[0]);
            remove_hr (s);
          }
        }
        else if (to_be_deleted (s, w[0], strtoul (w[1], NULL, 10), config))
        {
          Log (LL_INFO, "old partial file %s removed", w[0]);
          remove_hr (s);
        }
  ...
  if (!found)
  {
    Log (LL_INFO, "file not found, trying to create a tmpname");
...
fopen_again:
  if ((f = fopen (buf, "ab")) == 0)
  {
    Log (LL_CRIT, "%s: %s", buf, strerror (errno));
    return 0;
  }
  ...
  if (fstat (fileno (f), &sb) == 0)
  {
    ...
    {
      Log (LL_CRIT, "Partial size %" PRIuMAX " > %" PRIuMAX " (file size), delete partial",
    ...
    {
      Log (LL_CRIT, "no enough free space in %s (%luK, req-d %" PRIuMAX "K)",
    ...
    Log (LL_CRIT, "%s: fstat: %s", state->in.netname, strerror (errno));
...
int inb_reject (STATE *state, BINKD_CONFIG *config)
{
  ...
  {
    Log (LL_CRIT, "missing tmp file for %s!", state->in.netname);
    return 0;
  }
  else
  {
...
int check_pkthdr(STATE *state, char *netname, char *tmp_name,
  ...
  if ( (PKT = fopen(tmp_name, "rb")) == NULL )
      Log (LL_CRIT, "can't open file %s: %s, header check failed for %s", tmp_name, strerror (errno), netname);
  else if ( !fread(buf, sizeof(buf), 1, PKT) )
      Log (LL_CRIT, "file %s read error: %s, header check failed for %s", tmp_name, strerror (errno), netname);
  else if ( !pkt_getaddr(buf, &cz, &cn, &cf, &cp, NULL, NULL, NULL, NULL) )
      Log (LL_CRIT, "pkt %s version is %d, expected 2; header check failed", netname, buf[18]+buf[19]*0x100);
  else {
    check = 1;
    Log (LL_INFO, "pkt addr is %d:%d/%d.%d for %s", cz, cn, cf, cp, netname);
    /* do check */
    for (i = 0; i < state->nallfa; i++)
  ...
  if (check) Log (LL_CRIT, "bad pkt addr: %d:%d/%d.%d (file %s)", cz, cn, cf, cp, netname);
...
int inb_done (TFILE *file, STATE *state, BINKD_CONFIG *config)
{
  ...
  *real_name = 0;
  ...
  {
    Log (LL_CRIT, "missing tmp file for %s!", netname);
    return 0;
  }
  ...
  if (mask_test(netname, config->overwrite.first) && !ispkt(netname) && !isarcmail(netname))
  {
    for(i=0; ; i++)
    {
      ...
      {
        Log (LL_CRIT, "%s -> %s%s", netname, real_name, unlinked?" (overwrited)":"");
        break;
      }
      if ((errno != EEXIST && errno != EACCES && errno != EAGAIN) || i==10)
      {
        Log (LL_CRIT, "cannot rename %s to it's realname: %s! (data stored in %s)",
  ...
  } else
  {
    ...
    if (touch (tmp_name, file->time) != 0)
      Log (LL_CRIT, "touch %s: %s", tmp_name, strerror (errno));

    while (1)
    {
      if (!RENAME (tmp_name, real_name))
      {
        Log (LL_ERR, "%s -> %s", netname, real_name);
        break;
      }
      else
      {
        ...
        Log (LL_ERR, "error renaming `%s' to `%s': %s",
      ...
      {
        Log (LL_CRIT, "cannot rename %s to it's realname! (data stored in %s)",
             netname, tmp_name);
        *real_name = 0;
        return 0;
      }
  ...
  state->files_rcvd++;
  Log (LL_ERR, "rcvd: %s (%" PRIuMAX ", %.2f CPS, %s)", file->netname,
       (uintmax_t) file->size,
       (double) (file->size) /
