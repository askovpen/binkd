...
if (daemon(nochdir, 0) == -1)
	{ Log (LL_ERR, "Daemon() failed, %s", strerror(errno)); return -1; }
...
#endif
	{ Log (LL_ERR, "Setsid() failed, %s", strerror(errno)); }
...
if (fork() != 0) exit(0);
if (setpgrp() < 0)
	{ Log (LL_ERR, "Setpgrp failed, %s", strerror(errno)); }
...
{ int fd;
  if ((fd = open("/dev/tty", O_RDWR)) >= 0)
	...
	{ Log (LL_ERR, "Cannot open /dev/tty, %s", strerror(errno)); }
}
