...
/*
 * $Id: binlog.c,v 2.13 2010/05/22 08:11:30 gul Exp $
 *
 * Revision history:
 * $Log: binlog.c,v $
...
static void TLogStat (int status, STATE *state, char *binlogpath, int tzoff)
{
	...
	if (binlogpath[0]) {
		...
		if ((fl = fopen(binlogpath,"ab")) != NULL) {
			...
			ReleaseSem(&blsem);
			Log(LL_ERR, "unable to open binary log file `%s'",binlogpath);
		}
	}
...
static void FDLogStat (STATE *state, char *fdinhist, char *fdouthist, int tzoff)
{
	...
	{
		ReleaseSem(&blsem);
		Log (LL_ERR, "failed to write to %s", (state->to ? fdouthist : fdinhist));
	}
}
