...
/*
 * $Id: binkd.c,v 2.112 2012/05/14 06:14:57 gul Exp $
 *
 * $Log: binkd.c,v $
...
void chld (int *childcount)
{
...
#endif
  {
    ...
    if (childcount)
      childcount[0]--;
    if (WIFSIGNALED(status))
      Log (LL_ERR, "process %u exited by signal %u", pid, WTERMSIG(status));
    else
      Log (LL_NOTICE, "rc(%u)=%u", pid, WEXITSTATUS(status));
  }
}
	...
	switch (i)
	  {
	    ...
	    case 'i': /* install service */
	      Log (LL_CRIT,"Warning: switch \"-i\" is deprecated, use \"-t install\" instead");
	      if (isService()) break;
	    ...
	    case 'u': /* uninstall service */
	      Log (LL_CRIT,"Warning: switch \"-u\" is deprecated, use \"-t uninstall\" instead");
	      if (isService()) break;
	      if ( service_flag==w32_installservice
	        || service_flag==w32_uninstallservice
	        || service_flag==w32_queryservice
...
#endif
  if (optind<argc)
    Log (LL_CRIT, "Extra parameters ignored");
...
#endif
{
  ...
  if (service_flag==w32_run_as_service)
    Log (LL_NOTICE, "BEGIN service '%s', " MYNAME "/" MYVER "%s%s", service_name, get_os_string(), tmp);
  else
    Log (LL_NOTICE, "BEGIN standalone, " MYNAME "/" MYVER "%s%s", get_os_string(), tmp);
#else
  Log (LL_NOTICE, "BEGIN, " MYNAME "/" MYVER "%s%s", get_os_string(), tmp);
#endif
  if (sock_init ())
  ...
  if (current_config->zlib_dll[0]) {
    if (!zlib_init(current_config->zlib_dll))
      Log (LL_ERR, "cannot load %s, GZ compression disabled", current_config->zlib_dll);
    else
      Log (LL_DBG, "%s loaded successfully", current_config->zlib_dll);
  } else
    Log (current_config->zrules.first ? LL_WARN : LL_INFO, "zlib-dll not defined, GZ compression disabled");
#endif
#if defined(WITH_BZLIB2) && defined(ZLIBDL)
  if (current_config->bzlib2_dll[0]) {
    if (!bzlib2_init(current_config->bzlib2_dll))
      Log (LL_ERR, "cannot load %s, BZ2 compression disabled", current_config->bzlib2_dll);
    else
      Log (LL_DBG, "%s loaded successfully", current_config->bzlib2_dll);
  } else
    Log (current_config->zrules.first
#ifdef WITH_ZLIB
         && !zlib_loaded
#endif
         ? LL_WARN : LL_INFO, "bzlib2-dll not defined, BZ2 compression disabled");
#endif
  ...
  if (*current_config->pid_file)
  {
	...
	Log (LL_CRIT, "unexpected pid_file: %s: unlinked", current_config->pid_file);
    else
    {
	int current_log_level = LL_CRIT;
	switch ( errno )
	{
	   case ENOENT :	/* file not found or null pathname */
		current_log_level = LL_DBG3; /* it's ok */
		break;
	   default :
		break;
	}
    ...
    create_sem_file (current_config->pid_file, LL_CRIT);
  }
