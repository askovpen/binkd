...
/*
 * $Id: srif.c,v 2.21 2012/01/08 17:34:58 green Exp $
 *
 * $Log: srif.c,v $
...
int evt_test (EVTQ **eq, char *filename, EVT_FLAG *evt_flags)
{
  ...
  for (curr = evt_flags; curr; curr = curr->next)
  {
    if (pmatch_ncase(curr->pattern, filename))
    {
      if (curr->path)
      {
	Log (LL_NOTICE, "got %s, %screating %s", curr->pattern, curr->imm ? "" : "delayed ", curr->path);
	if (curr->imm)
	{
	  ...
	  *eq = evt_queue(*eq, 'f', curr->path);
      ...
      {
	Log (LL_NOTICE, "got %s, %sstarting %s", curr->pattern, curr->imm ? "" : "delayed ", curr->command);
	rc=1;
      }
    }
  }
...
void evt_set (EVTQ *eq)
{
  ...
  while (eq)
  {
    if (eq->evt_type == 'e')
    {
      Log (LL_NOTICE, "Running %s", eq->path);
      run(eq->path);
    }
    else
    {
      Log (LL_NOTICE, "Creating %s", eq->path);
...
static FTNQ *parse_response (FTNQ *q, char *rsp, FTN_ADDR *fa, BINKD_CONFIG *config)
{
  ...
  if (in)
  {
	...
	break;
      ...
      buf[i] = 0;
      Log (LL_NOTICE, "parse_response: add file `%s' to queue", buf + 1);
      switch (*buf)
	{
	  case '=':
	  ...
	  case '-':
	    ...
	    break;
	  default:
	    Log (LL_ERR, "parse_response: unknown predictor `%c', ignored response file `%s'", *buf, buf + 1);
	    break;
	}
    }
    if (ftell (in) == 0)
	Log (LL_WARN, "SRIF response file is empty");
    fclose (in);
  }
  else
    Log (LL_CRIT, "parse_response: %s: %s", rsp, strerror (errno));
  return q;
}
...
static EVTQ *run_args(EVTQ *eq, char *cmd, char *filename0, STATE *st, int imm)
{
  ...
  {
    fn="-";
    Log (LL_NOTICE, "Invalid filename (%s) received!", filename0);
    break;
  }
  for (i=0; pn[i]; i++)
  ...
  for (l=0; (l<strlen(w)) && ((sp=strchr(w+l, '*'))!=NULL); l++)
  {
    ...
    switch(toupper(sp[1]))
    {
      case 'N': /* short filename for win32 */
#ifdef WIN32
        {
          ...
          if ((i<1) || (i>MAXPATHLEN))
          {
            Log (LL_ERR, "GetShortPathName() fails %d", GetLastError());
            use_fn=1;
            strcpy(ts, fn);
          }
          w = ed (w, "*N", ts, &sw);
	...
	if (sp[2] == '@' || sp[2] == '*')
	{
	  ...
	  if (addrlist == NULL)
	  { Log (LL_CRIT, "Not enough memory, %ld bytes needed", st->nfa*(FTN_ADDR_SZ+1l));
	    break;
	  }
	  for (i=0; i<st->nfa; i++)
	  { if (pa != addrlist) *pa++=' ';
  ...
  if ((fn!=filename0) && (use_fn))
    Log (LL_CRIT, "Security problem. Execution aborted...");
