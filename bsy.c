 ...
 */
/*
 * $Id: bsy.c,v 2.11 2010/06/15 20:24:48 gul Exp $
 *
 * $Log: bsy.c,v $
...
int bsy_add (FTN_ADDR *fa0, bsy_t bt, BINKD_CONFIG *config)
{
  ...
  if (*buf)
  {
    ...
    if (mkpath (buf) == -1)
      Log (LL_ERR, "mkpath('%s'): %s", buf, strerror (errno));
    ...
    if (create_sem_file (buf, 5))
    {
      ...
      if (new_bsy->h == -1)
        Log (LL_WARN, "Can't open %s: %s!", buf, strerror(errno));
#if defined(OS2)
      else
...
void bsy_remove (FTN_ADDR *fa0, bsy_t bt, BINKD_CONFIG *config)
{
  ...
  if (*buf)
  {
	...
	if (bsy->h != -1)
	  if (close(bsy->h))
            Log (LL_WARN, "Can't close %s (handle %d): %s!", buf, bsy->h, strerror(errno));
#endif
	delete (buf);
...
void bsy_touch (BINKD_CONFIG *config)
{
  ...
  if (time (0) - last_touch > BSY_TOUCH_DELAY)
  {
	...
	if (touch (buf, time (0)) == -1)
          Log (LL_ERR, "touch %s: %s", buf, strerror (errno));
	else
