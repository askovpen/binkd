...
/*
 * $Id: server.c,v 2.54 2012/05/14 06:14:59 gul Exp $
 *
 * $Log: server.c,v $
  ...
  for (ai = aiHead; ai != NULL && sockfd_used < MAX_LISTENSOCK; ai = ai->ai_next)
  {
    ...
    if (ai->ai_family == PF_INET6)
    {
      ...
      if (setsockopt(sockfd[sockfd_used], IPPROTO_IPV6, IPV6_V6ONLY, 
        ...
        Log (LL_CRIT, "servmgr setsockopt (IPV6_V6ONLY): %s", TCPERR());
    }
#endif
    if (setsockopt (sockfd[sockfd_used], SOL_SOCKET, SO_REUSEADDR,
                  (char *) &opt, sizeof opt) == SOCKET_ERROR)
      Log (LL_CRIT, "servmgr setsockopt (SO_REUSEADDR): %s", TCPERR ());
  ...
  for (;;)
  {
        ...
        if (checkcfg()) 
	{
          ...
          if (checkcfg())
	  {
        ...
        if (!binkd_exit) /* Suppress servmgr socket error at binkd exit */
          Log (LL_CRIT, "servmgr select(): %s", TCPERR ());
        goto accepterr;
    }
    ...
    for (curfd=0; curfd<sockfd_used; curfd++)
    {
                                ...
                                &client_addr_len)) == INVALID_SOCKET)
      {
        ...
        {
          if (!binkd_exit)
            Log (LL_CRIT, "servmgr accept(): %s", TCPERR ());
#ifdef UNIX
	...
	if (aiErr == 0) 
          Log (LL_WARN, "incoming from %s (%s)", host, service);
        else
        {
          Log (LL_ERR, "Error in getnameinfo(): %s (%d)", gai_strerror(aiErr), aiErr);
	  Log (LL_WARN, "incoming from unknown");
	}
        ...
        if ((pid = branch (serv, (void *) &new_sockfd, sizeof (new_sockfd))) < 0)
        {
          ...
          threadsafe(--n_servers);
          PostSem(&eothread);
          Log (LL_CRIT, "servmgr branch(): cannot branch out");
          sleep(1);
        }
        else
        {
          Log (LL_INFO, "started server #%i, id=%i", n_servers, pid);
#ifdef HAVE_FORK
          soclose (new_sockfd);
#endif
        }
